...
import Foundation
    ...
    public final class WatchConnectivityService: NSObject, WCSessionDelegate {
        ...
        public func send(message: ConnectivityMessage) {
            ...
            #if DEBUG
                print(
                    "[WC] watch send state activationState=\(session.activationState.rawValue) isReachable=\(session.isReachable) isCompanionAppInstalled=\(session.isCompanionAppInstalled)"
                )
            #endif
        ...
        public func session(
            ...
            #if DEBUG
                print(
                    "[WC] watch activationDidComplete state=\(activationState.rawValue) error=\(error?.localizedDescription ?? "nil") isReachable=\(session.isReachable)"
                )
            #endif
        }
    ...
    public final class WatchConnectivityService {
        ...
        private init() {}
        public func activate() {}
        public func send(message _: ConnectivityMessage) {}
    }
#endif
