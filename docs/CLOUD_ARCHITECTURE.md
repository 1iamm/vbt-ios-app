...
- **AI 训练**：默认关 + onboarding 引导开启 + 完全脱敏
...
| AI 训练数据收集 | 默认关 + onboarding 引导 | 合规更稳，App Store 审核更顺 |
...
用户 onboarding 看到两个开关：
```
☐ 启用云同步         （默认关）
☐ 帮助改进 AI         （默认关，必须先开云同步才能开此项）
```
...
### Q: 我不开云同步会失去什么？
**A**: 几乎不失去任何核心功能。仅失去：换手机数据迁移、未来 AI 私教（V2）、跨设备同步。
